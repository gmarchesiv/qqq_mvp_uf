...
def update_status(app, vars,varsApp, params):
    ...
    vars.bloqueo = config.get("bloqueo", True)
...
# REGISTRO DE STRIKES
def registro_strike_proximo(app, vars, params):
    ...
    vars.precio = app.etfs[5]["price"]
   
    printStamp(f"PRECIO: {app.etfs[5]['price']} $")

    call = int(vars.precio * ((100 + params.rangos_strikes[0][1]) / 100))
    put = int(vars.precio * ((100 - params.rangos_strikes[0][1]) / 100))

    call_inf = int(vars.precio * ((100 + params.rangos_strikes[0][0]) / 100))
    put_inf = int(vars.precio * ((100 - params.rangos_strikes[0][0]) / 100))
...
def registro_strike_proximo_2 (app, vars, params): 
    ...
    vars.call_close_2 = cbid
    vars.put_close_2 = pbid
    print("===============================================")
    printStamp(
